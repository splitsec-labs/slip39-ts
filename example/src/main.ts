import { Slip39, decodeHexString, encodeHexString } from "slip39-ts";
...
function printShares(shares: string[]): void {
  shares.forEach((s, i) => {
...
console.log(
  `* Successfully generated Slip39 object: ${JSON.stringify(slip, null, 2)}`,
);
...
const aliceBothGroupShares = slip
  .fromPath("r/0/0")
...
console.log(
  `\n* Shares used by Alice alone for restoring the master secret (total of ${String(requiredGroupShares.length)} member-shares):`,
);
...
console.log(
  `\n* Shares used by Alice + 3 friends for restoring the master secret (total of ${String(requiredGroupShares.length)} member-shares):`,
);
printShares(requiredGroupShares);
await recover(requiredGroupShares, passphrase);
...
console.log(
  `\n* Shares used by Alice + 2 family members for restoring the master secret (total of ${String(requiredGroupShares.length)} member-shares):`,
);
printShares(requiredGroupShares);
await recover(requiredGroupShares, passphrase);
...
console.log(
  `\n* Shares used by 3 friends + 2 family members for restoring the master secret (total of ${String(requiredGroupShares.length)} member-shares):`,
);
printShares(requiredGroupShares);
await recover(requiredGroupShares, passphrase);
